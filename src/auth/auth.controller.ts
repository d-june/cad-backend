import { Controller, Query, Post, Body, Get, BadRequestException, UnauthorizedException, Res, HttpStatus, Req } from '@nestjs/common';
...
import { UseInterceptors, UseGuards } from '@nestjs/common/decorators';
...
export class AuthController {
    ...
    async register(@Body() dto: RegisterDto){
        ...
        if(!user) {
            throw new BadRequestException(`Не получается зарегистрировать пользлвателя с данными ${JSON.stringify(dto)}`)
        }
